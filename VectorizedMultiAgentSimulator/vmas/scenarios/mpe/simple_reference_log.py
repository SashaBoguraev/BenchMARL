...
import torch, random, os, csv, math
...
class Scenario(BaseScenario):
    ...
    def observation(self, agent: Agent):
        agent_comm_list = agent.state.c.tolist()
        if len(agent_comm_list) == 1 and [0.0]*10 not in agent_comm_list:
            agent_comm_list = agent_comm_list[0]
            agent_comm_list.append(-self.reward(agent).item())
            # Get file_address
            # file_address = "/Users/sashaboguraev/Desktop/Cornell/College Scholar/BenchMARL/evaluation/comms/Noise/Idiolect/LanguageB/env2/"
            file_address = "/Users/sashaboguraev/Desktop/Cornell/College Scholar/BenchMARL/evaluation/comms/dummysheet"
            if not os.path.isdir(file_address):
                os.makedirs(file_address)
            
            # Open the file with the append state
            with open(file_address+agent.name+'_states.csv', mode='a') as file:
                writer = csv.writer(file)
                # Write the state information to the CSV file
                writer.writerow(agent_comm_list)
