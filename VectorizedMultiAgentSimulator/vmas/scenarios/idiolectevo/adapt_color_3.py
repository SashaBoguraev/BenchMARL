...
class Scenario(BaseScenario):
    ...
    def reset_world_at(self, env_index: int = None):
        if env_index is None:
            ...
            colors = [
                torch.tensor(
                    [5.82358321e-01, 1.46604746e-01, 6.51536714e-04], device=self.world.device, dtype=torch.float32
                ),
                torch.tensor(
                    [8.43802356e-01, 5.91663097e-01, 7.70336708e-01], device=self.world.device, dtype=torch.float32
                ),
                torch.tensor(
                    [6.07103781e-02, 9.54713269e-01, 4.00395304e-01], device=self.world.device, dtype=torch.float32
                )
            ]
